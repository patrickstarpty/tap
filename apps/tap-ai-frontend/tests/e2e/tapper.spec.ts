...
test("Library uploads/status and Project API recovery, answers, citations, scope, digests and deletion", async ({
  ...
  const initialDocumentIds = new Set(await documentIds(page));
  const journeyDocumentIds = async () =>
    (await documentIds(page)).filter((id) => !initialDocumentIds.has(id));
  ...
  expect((await journeyDocumentIds()).sort()).toEqual(baselineIds);
  ...
  await expect
    .poll(async () => (await journeyDocumentIds()).length, { timeout: 45_000 })
    .toBe(7);
