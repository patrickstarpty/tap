...
module.exports = {
  forbidden: [
    {
      name: "tap-ai-does-not-import-tap-implementation",
      severity: "error",
      from: { path: "^src/", pathNot: "\\.test\\.[tj]sx?$" },
      to: {
        path: "(?:prototype/(?:automation/|testManagement/|TestAnalytics|testAnalytics|floatingAssistantModel|TapperFloatingAssistant|artifacts/(?:fixtures|state|model))|apps/web/src|\\.\\./web/src)",
      },
    },
    {
      name: "no-circular",
