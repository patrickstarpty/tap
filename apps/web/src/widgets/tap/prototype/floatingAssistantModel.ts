...
import type { AutomationWorkspaceView } from "../../../legacy/automation/AutomationWorkspace";
...
export function createFloatingAssistantReply(
  ...
  const guidance = {
    ...
    run: {
      note: t(
        "Confirm the execution target and review these checks before starting a run.",
        "开始运行前，请确认执行目标并完成以下检查。",
      ),
      suggestions: [
        t(
    ...
    unsupported: {
      note: t(
        "The current page does not contain enough information to answer this question. Add relevant knowledge or choose a page review below.",
        "当前页面的信息不足以回答这个问题。请补充相关知识，或选择下方的页面检查。",
      ),
