...
def test_native_junit_requires_every_selected_domain(gate):
    ...
    raw = junit([("tests.contract.test_origin_policy", "test_origin", None)])
    with pytest.raises(ValueError):
        gate.parse_junit(raw, files)
    raw = junit(
        [
            ("tests.contract.test_origin_policy", "test_origin", None),
            ("tests.unit.access.test_scope_context", "test_scope", None),
        ]
    )
...
def test_native_junit_rejects_skip_xfail_xpass_and_retry_even_when_counts_claim_pass(gate, outcome):
    raw = junit([("tests.contract.test_origin_policy", "test_origin", outcome)])
    with pytest.raises(ValueError):
...
def test_native_junit_rejects_missing_real_case_and_forged_module(gate):
    raw = junit([("evil.tests.contract.test_origin_policy", "test_origin", None)])
    with pytest.raises(ValueError):
        gate.parse_junit(raw, ["contract/test_origin_policy.py"])
    raw = junit([("tests.contract.test_origin_policy", "test_other", None)])
    with pytest.raises(ValueError):
        gate.parse_junit(
            raw,
    ...
    for expected in gate.command_registry(tmp_path):
        ...
        if row["kind"] == "schema":
            ...
            for file in gate.BUNDLES[name]:
                classname = "tests." + file[:-3].replace("/", ".")
    ...
    def failed_child(argv, env, log):
        ...
        xml = ET.fromstring(
            junit(
                [
                    (
                        "tests.integration.test_validation_identity_registry",
                        "test_fixture",
                        "error",
                    )
                ],
