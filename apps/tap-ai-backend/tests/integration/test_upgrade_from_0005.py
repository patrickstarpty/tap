...
def test_invalid_revision_cli_exits_nonzero_with_a_safe_report() -> None:
    result = subprocess.run(
        ...
        env={**os.environ, "PYTHONPATH": str(ROOT / "apps/tap-ai-backend/src")},
        capture_output=True,
        text=True,
        check=False,
    )
...
def test_0014_test_management_revision_is_literal_and_registered():
    from scripts.migration_support import validate_revision
...
def test_0010_corruption_partial_state_multisource_and_downgrade_guards(owned_project_mysql):
    ...
    migration_path = ROOT / "apps/tap-ai-backend/migrations/versions/0010_knowledge_sources.py"
