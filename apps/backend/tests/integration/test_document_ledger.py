...
def test_concurrent_renamed_duplicate_has_one_identity_and_one_dispatch_path() -> None:
    async def scenario() -> None:
        ...
        await clean(engine)
        try:
            first_repo = MysqlDocumentRepository(sessions, scope=VALIDATION_SCOPE)
            second_repo = MysqlDocumentRepository(sessions, scope=VALIDATION_SCOPE)
            digest = "d" * 64
    ...
    async def scenario() -> None:
        ...
        try:
            ...
            first, second = await asyncio.gather(
                MysqlDocumentRepository(sessions, scope=VALIDATION_SCOPE).reserve_upload(
...
def test_lease_token_ownership_and_expired_lease_recovery_are_conditional() -> None:
    async def scenario() -> None:
        ...
        try:
            ...
            recovered = await MysqlDocumentRepository(sessions, scope=VALIDATION_SCOPE).claim_jobs(
                worker_id="worker-b",
    ...
    async def scenario() -> None:
        ...
        try:
            with pytest.raises(ValueError):
                await MysqlDocumentRepository(sessions, scope=VALIDATION_SCOPE).claim_jobs(
...
def test_claim_lease_is_based_on_post_lock_database_time() -> None:
    ...
    async def scenario() -> None:
        ...
        await clean(engine)
        try:
            setup_repository = MysqlDocumentRepository(setup_sessions, scope=VALIDATION_SCOPE)
            ...
            claimed = (
                await MysqlDocumentRepository(delayed_sessions, scope=VALIDATION_SCOPE).claim_jobs(
...
def test_retry_and_delete_share_job_then_document_lock_order_without_deadlock() -> None:
    ...
    async def scenario() -> None:
        ...
        await clean(engine)
        try:
            setup = MysqlDocumentRepository(setup_sessions, scope=VALIDATION_SCOPE)
            ...
            retry = asyncio.create_task(
                MysqlDocumentRepository(retry_sessions, scope=VALIDATION_SCOPE).retry_failed(
            ...
            deleting = asyncio.create_task(
                MysqlDocumentRepository(delete_sessions, scope=VALIDATION_SCOPE).request_delete(
...
def test_late_outbox_failure_keeps_promoted_locator_for_durable_takeover() -> None:
    async def scenario() -> None:
        ...
        await clean(engine)
        try:
            repository = OutboxFailureRepository(sessions, scope=VALIDATION_SCOPE)
