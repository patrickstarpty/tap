...
V0 已完成 authoritative migration metadata、Task 2A 固定验证身份与 Task 2B Project 数据回填：Alembic 显式装载 17 张表，共同授权在既有检索 gate 前核对有效 Enterprise/Project/Actor；`0007` 为原 14 张业务表增加范围约束，仓储强制绑定可信 Project，Outbox 保存 canonical compatibility event。证据见[身份验收](../reviews/2026-09-05-tapper-v0-identity-review.md)与[Project 隔离验收](../reviews/2026-09-05-tapper-v0-project-scope-review.md)；后者保留完整回归的一项待复核失败。领域事件/Problem、HTTP 范围接入、Audit、可靠性恢复与 V0 出口仍在后续任务中。
