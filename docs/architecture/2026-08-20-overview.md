...
| 本地验证切片 | Tapper 来源优先 `doc` 问答；React/Vite + FastAPI + MySQL/Redis/Azurite/Milvus/LiteLLM；仅 loopback、无认证、无 OCR |
| 证据边界     | 已恢复 `engprod` 三条讨论；长回复存在源端截断，详见 [来源说明](../reference/2026-08-20-source-notes.md)            |

> 本文保留 2026-08-20 至 2026-09-02 的 Azure/Git/Intelligence-first 架构演进，不再是当前规范性基线。当前架构见 [Tapper 知识与 Web 自动化平台架构 v0.4](2026-09-04-tapper-knowledge-web-automation-overview.md)，正式范围由 [RFC-009](../proposals/2026-09-04-rfc-009-tapper-knowledge-web-automation-platform.md) 与 ADR-020–025 管理。若本文与当前基线冲突，以后者为准。
...
下图只表达已批准但尚未实施的 P1.0–P1.2 Intelligence 增量。它运行在现有 loopback Tapper 旁，不需要 Project、Release、源码、Test Management 或真实测试执行；虚线 Codex 路径必须显式 opt-in，默认评测和本地模式使用确定性 Runtime。
...
flowchart TB
    ...
    Context -->|optional ready sources| Knowledge[Tapper Knowledge API]
...
%%{init: {
  "theme": "base",
  "themeVariables": {"fontFamily": "Arial, Noto Sans SC, sans-serif", "fontSize": "15px", "primaryColor": "#EEE9FA", "primaryTextColor": "#263445", "primaryBorderColor": "#94A3B8", "secondaryColor": "#E7F1FA", "secondaryTextColor": "#263445", "secondaryBorderColor": "#94A3B8", "tertiaryColor": "#FFF4D6", "tertiaryTextColor": "#263445", "tertiaryBorderColor": "#94A3B8", "lineColor": "#7E8B9B", "textColor": "#263445", "mainBkg": "#EEE9FA", "nodeBorder": "#94A3B8", "clusterBkg": "#F7F8FC", "clusterBorder": "#AAB4C2", "edgeLabelBackground": "#FFFFFF", "background": "#FFFFFF", "actorBkg": "#EEE9FA", "actorBorder": "#94A3B8", "actorTextColor": "#263445", "actorLineColor": "#AAB4C2", "signalColor": "#7E8B9B", "signalTextColor": "#263445", "labelBoxBkgColor": "#E7F1FA", "labelBoxBorderColor": "#94A3B8", "labelTextColor": "#263445", "loopTextColor": "#263445", "noteBkgColor": "#FFF4D6", "noteBorderColor": "#CDBD87", "noteTextColor": "#263445", "activationBkgColor": "#E7F1FA", "activationBorderColor": "#94A3B8", "attributeBackgroundColorOdd": "#F7F8FC", "attributeBackgroundColorEven": "#FFFFFF"},
  "flowchart": {"curve": "linear", "nodeSpacing": 16, "rankSpacing": 40, "padding": 10, "wrappingWidth": 110}
}}%%
flowchart TB
    ...
    classDef default fill:#EEE9FA,stroke:#94A3B8,stroke-width:1px,color:#263445;
    classDef data fill:#E7F1FA,stroke:#94A3B8,stroke-width:1px,color:#263445;
    class GH,Identity,State,Stream,Report,Candidate,Git,MySQL,Redis,Search,Blob,BS,KV,Chat,Coder,Embed,Rerank,Vision data;
    classDef action fill:#FFF4D6,stroke:#94A3B8,stroke-width:1px,color:#263445;
    class User,Assist,Workbench,AdminApproval,PublishApproval action;
...
Tapper 的 RAG/Citation 是未来平台共享的 Knowledge Plane 基础，但当前不以完整企业 RAG 或 Knowledge Chat 作为出口。Intelligence Layer 先通过 Workspace Scope、Context Snapshot、Task、Artifact 和 Review 验证价值；后续 Project、Release、Test Asset 和 Run 只通过 typed relation 与它关联，不成为启动任务的强制父对象。
...
| Knowledge/Citation | 复用 Tapper `doc` 基础；按需补强                                                                   | 接入测试资产与失败数据                | 共享检索与授权                | 四索引与规模化调优           |
...
1. **Knowledge Plane 可独立工作**：关闭 Intelligence Runtime、Test IR 或执行网格时，Tapper 摄取、检索、回答和引用回归仍完整可用。
...
flowchart TB
    ...
    subgraph Async[Python Async and Worker Roles]
      Ingestion[Existing Tapper Ingestion Worker]
    ...
    classDef default fill:#EEE9FA,stroke:#94A3B8,stroke-width:1px,color:#263445;
    classDef data fill:#E7F1FA,stroke:#94A3B8,stroke-width:1px,color:#263445;
    class MySQL,Redis,Blob,Search,Model data;
    classDef action fill:#FFF4D6,stroke:#94A3B8,stroke-width:1px,color:#263445;
    class Shell,Tapper,Lab,FutureUI action;
    style Browser fill:#F7F8FC,stroke:#AAB4C2,stroke-width:1px,stroke-dasharray:6 4,color:#263445;
    ...
    style Specialist fill:#F7F8FC,stroke:#AAB4C2,stroke-width:1px,stroke-dasharray:6 4,color:#263445;
```
...
| 模块                        | 用户能力                                                          | 后端边界                                   | 阶段                           |
| --------------------------- | ----------------------------------------------------------------- | ------------------------------------------ | ------------------------------ |
| Tapper Ask & Library        | 上传、摄取状态、选择 ready 来源、限定来源问答与 Citation          | Knowledge/Document/Answer/Citation         | 已实现本地基线                 |
...
## 4. TAP AI Orchestrator

当前 P1.2 只实现版本化、可恢复的 Intelligence Task Control。[ADR-029](../decisions/2026-09-18-adr-029-langgraph-ai-interaction-task-orchestrator.md) 将后续目标固定为 TAP AI Chat 与 AI Task 共用的版本化 LangGraph；它不交付独立 Query Router、Model Router、通用多 Agent 平台或自由规划器。以下能力不得成为 P1.0–P1.2 的隐式依赖。

### 4.1 两条执行轴与三种剖面

编排器分别记录执行生命周期与推理方式，避免把“运行时间长”错误等同于“推理复杂”：

| 维度 | 取值 | 说明 |
| ---- | ---- | ---- |
| `execution_mode` | `inline \| durable` | 在线完成，或使用 Task/GraphRun、checkpoint、Outbox、Redis 唤醒和 Worker 可恢复执行 |
| `reasoning_mode` | `direct \| workflow \| agentic` | 直接生成、确定性工作流，或受预算限制的动态规划与工具循环 |

用户侧使用三个常用执行剖面：Fast Chat 默认 `inline + direct`；Durable Workflow 默认 `durable + workflow`；Bounded Agentic Task 使用 `agentic`，超过在线预算、等待外部结果或需要人工确认时复用 durable runtime。长任务不必进入 Agentic Loop，复杂任务也不能用 Agentic Loop 替代可靠队列、状态机、租约和外部副作用幂等。

Loop 不能绕开平台状态机。每个动态动作仍要落为有类型的 Task/GraphRun/Attempt，并受 deadline、token、费用、工具和网络策略约束。开放探索得出的方案最终进入响应核验或人工审批，再转入确定性验证。
...
flowchart TB
    Goal[TAP AI Chat / AI Task] --> Admission[Task Classification & Admission]
    Admission -->|inline + direct| Fast[Fast Chat]
    Admission -->|durable + workflow| Durable[Durable Workflow]
    Admission -->|bounded agentic| Agentic[Bounded Agentic Task]
    Fast --> Ports[Governed Capability Ports]
    Durable --> Runtime[Task / GraphRun / Checkpoint / Worker]
    Runtime --> Ports
    Agentic -->|inline or durable| Runtime
    Agentic --> Ports
    Ports --> Verify[Response / Result Verification]
    Verify --> Summary[Answer / Progress / Approval]

    classDef default fill:#EEE9FA,stroke:#94A3B8,stroke-width:1px,color:#263445;
    classDef action fill:#FFF4D6,stroke:#94A3B8,stroke-width:1px,color:#263445;
    class Goal,Summary action;
```
...
3. **执行规划**：选择 Fast Chat、Durable Workflow 或 Bounded Agentic Task，记录两条执行轴，构建依赖、预算、完成条件，并仅在 Agentic 剖面内受控重规划。
4. **任务调度**：管理 Task/GraphRun、租约、优先级、串并行、暂停、恢复、取消和统一汇总；不提供跨产品通用 Agent 注册中心。
5. **上下文管理**：最小装载、压缩、滑动窗口和跨 Agent 受控共享。
6. **能力发现**：只查询 TAP AI 获准模型、工具和 Provider 的能力、版本与健康状态；业务动作始终通过正式领域端口。
...
flowchart TB
    ...
    classDef default fill:#EEE9FA,stroke:#94A3B8,stroke-width:1px,color:#263445;
    classDef data fill:#E7F1FA,stroke:#94A3B8,stroke-width:1px,color:#263445;
    class LocalCommit,PR,Feedback data;
    classDef action fill:#FFF4D6,stroke:#94A3B8,stroke-width:1px,color:#263445;
    class New,Existing,Approval action;
```
...
flowchart TB
    ...
    classDef default fill:#EEE9FA,stroke:#94A3B8,stroke-width:1px,color:#263445;
    classDef data fill:#E7F1FA,stroke:#94A3B8,stroke-width:1px,color:#263445;
    class Provider,Blob,State,Outbox data;
```
...
flowchart TB
    D[Document ingestion] --> E[LiteLLM tapper-embedding]
    Q[User query] --> E
    E --> A[Alibaba text-embedding-v4<br/>1536D zh / en / mixed]
    A --> M[(Local Milvus doc projection)]
    M --> V[Bounded selected Evidence]
    V --> B{TAPPER_ANSWER_BACKEND}
    B -->|litellm - default| L[LiteLLM tapper-chat]
...
因此，本地 Tapper 是对来源建设、文档 ingestion、grounded answer 与 citation 完整性的已交付验证，不是完整 Knowledge Chat 或企业 RAG 的完成声明。下面的 Azure AI Search 四索引、Entra 权限、正式 Chat/SSE、AKS 与生产门禁是后置 Knowledge Plane/企业平台设计，不是当前 Intelligence Phase 1 的出口。
...
flowchart TB
    ...
    classDef default fill:#EEE9FA,stroke:#94A3B8,stroke-width:1px,color:#263445;
    classDef data fill:#E7F1FA,stroke:#94A3B8,stroke-width:1px,color:#263445;
    class Source,Ledger,Queue,Physical,Checkpoint,Alias data;
```
...
flowchart TB
    ...
    classDef default fill:#EEE9FA,stroke:#94A3B8,stroke-width:1px,color:#263445;
    classDef data fill:#E7F1FA,stroke:#94A3B8,stroke-width:1px,color:#263445;
    class Git,MySQL,Outbox,Redis,Blob,Search data;
```
...
- LiteLLM 可按企业 Model Policy 执行协议归一化、provider routing、超时、重试和显式批准的 provider fallback；Tapper 本地 `.env` 选择的 LiteLLM/Codex 回答后端不适用该 fallback，二者严格独占并在漂移时 fail closed。
...
Tapper 本地已实现路径是该企业目标之外的窄例外：LiteLLM 始终承担文档/query Embedding，回答端口才由 `.env` 在 LiteLLM 与精确本机 Codex CLI 之间选择。该 `doc` Milvus 投影和个人 ChatGPT 登录不代表 Azure AI Search 四索引、Entra、共享凭据或完整 Phase 1 已交付。
...
flowchart LR
    ...
    classDef default fill:#EEE9FA,stroke:#94A3B8,stroke-width:1px,color:#263445;
    classDef data fill:#E7F1FA,stroke:#94A3B8,stroke-width:1px,color:#263445;
    class Identity,Decision,Untrusted data;
```
...
flowchart LR
    ...
    classDef default fill:#EEE9FA,stroke:#94A3B8,stroke-width:1px,color:#263445;
    classDef data fill:#E7F1FA,stroke:#94A3B8,stroke-width:1px,color:#263445;
    class Entra,GitHub,MySQL,Redis,Search,Blob,KV,DeviceHost,BS,Provider data;
    classDef action fill:#FFF4D6,stroke:#94A3B8,stroke-width:1px,color:#263445;
    class User,Web action;
    style VNet fill:#F7F8FC,stroke:#AAB4C2,stroke-width:1px,stroke-dasharray:6 4,color:#263445;
    style AKS fill:#F7F8FC,stroke:#AAB4C2,stroke-width:1px,stroke-dasharray:6 4,color:#263445;
    style EdgeNS fill:#F7F8FC,stroke:#AAB4C2,stroke-width:1px,stroke-dasharray:6 4,color:#263445;
    style ApiNS fill:#F7F8FC,stroke:#AAB4C2,stroke-width:1px,stroke-dasharray:6 4,color:#263445;
    style KnowledgeNS fill:#F7F8FC,stroke:#AAB4C2,stroke-width:1px,stroke-dasharray:6 4,color:#263445;
    style ControlNS fill:#F7F8FC,stroke:#AAB4C2,stroke-width:1px,stroke-dasharray:6 4,color:#263445;
    style AgentNS fill:#F7F8FC,stroke:#AAB4C2,stroke-width:1px,stroke-dasharray:6 4,color:#263445;
    style ExecNS fill:#F7F8FC,stroke:#AAB4C2,stroke-width:1px,stroke-dasharray:6 4,color:#263445;
    style ModelNS fill:#F7F8FC,stroke:#AAB4C2,stroke-width:1px,stroke-dasharray:6 4,color:#263445;
    style ObsNS fill:#F7F8FC,stroke:#AAB4C2,stroke-width:1px,stroke-dasharray:6 4,color:#263445;
```
