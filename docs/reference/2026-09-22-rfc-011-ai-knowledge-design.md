...
> **范围标注（2026-09-22）：近期聚焦知识问答及基础 Insights 的 AI 查询与解读。** 本文已有技术选择、处理流程和部分契约约束，仍需按试点细化首批资料类型、接口/状态迁移、异常交互和实施任务。测试生成扩展、完整 Skills/Agents 资源包、主动订阅/建议收件箱及正式行动闭环保留为后续目标；下文“首批”“首版”描述相应能力启用时的目标，不代表全部列入本轮交付。统一编排仅推进这两条近期路径所需部分。
...
当前先聚焦知识问答与基础 Insights，原第 1–2 批中的完整主动闭环保留后续目标，不作为近期交付条件。后续先补齐知识事件、准入/记忆、影响草稿、收件箱/单次批准的详细设计，再依赖真实 Run/Insights 扩展回归、失败 RCA 与动作反馈；周期提醒/预授权在降噪试点后逐项启用。按项目开关回滚，保审计/资产。
