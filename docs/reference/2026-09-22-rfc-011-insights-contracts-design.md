...
> **范围标注（2026-09-22）：近期聚焦基础 Test Insights，完整 Insights 与协作为后续阶段，详细设计待补。** 本文已有数据链、指标公式、去重更正和授权恢复约束，但尚需固定首个真实报告来源、字段映射、表结构/API、页面交互及实施任务；不能视为完整可开工设计。近期范围见下表，完整质量评分、发布门禁、多源工程连接和协作不作为基础版验收条件。

| 范围 | 当前安排 |
| --- | --- |
| 真实报告接入、原件/接收账本、身份映射、去重与更正、ClickHouse、Insights API | 近期；先选已有 CI 或外部报告来源打通，不依赖平台自建 Web/App 执行 |
| 运行列表与证据钻取、首次/最终通过率、失败分布、耗时与趋势、AI 结合知识解释 | 近期；固定首批指标和查询口径，明确缺数、样本不足与事实/推断，AI 不代替权威统计 |
| 五类完整质量分析、综合评分、发布检查、规则告警、完整失败协作与多源连接器 | 后续第 4 阶段；已有目标与规则约束，逐来源映射、接口与交互详细设计待补 |
| Web/App/负载调度、Jira 写入、主动订阅及自动行动 | 后续能力；下文跨产品契约保留目标边界，不表示近期必须实现 |
...
以下对应收窄后的近期范围和后续依赖，非工期承诺；实施按项目增量启用保既有数据，模拟数据不混真实报告。基础 Insights 先接已有 CI 或外部真实报告，不依赖平台自建自动化。

| 阶段 | 本专题相关完成条件 |
| --- | --- |
| 共同前置（与近期路径并行） | 身份/项目映射、来源测试身份、Conversation/Turn 与必要 Task/GraphRun、工具/服务身份/契约版本、凭证；真实资料/报告/环境、恢复幂等、备份和模型范围实测。独立用例库不作为外部报告接入前提 |
| 第 1 阶段近期子集：知识问答 | 资料核对/发布、Fast Chat、检索与可定位引用；必要长任务恢复和权限验证。测试管理扩展另行设计与排期 |
| 第 2 阶段近期子集：基础 Insights | 真实报告→账本/原件→映射去重→ClickHouse→指标/API/页面/AI 解读，重复不重计、缺数可见；结合知识的分析不依赖 chDB。自建 Web Run、自动结果回写和主动行动闭环后续建设 |
| 第 3–4 阶段：后续、详细设计待补 | Android/iOS 分别实测；完整五域指标/评分/规则/告警、工程连接、Forge 身份和双方授权逐项验收 |
| 第 5 阶段：后续、详细设计待补 | 负载协议闭环后扩浏览器/混合、分布式/私网；全局统计、失联停止和缺数据门禁接同一 Insights |
