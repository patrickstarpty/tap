...
原路线将用例/评审/计划/手工执行、导入和基础 Jira REST 放在第 1 批，自动化结果映射放在第 2 批；当前已收窄近期范围，上述新增能力均移至后续，补齐详细设计后另定排期。Forge 内嵌仍属于后续第 4 阶段。逐页范围与产品限制见 [测试管理研究](../reviews/2026-09-17-browserstack-test-management-page-analysis.md)。
...
L1–L3 都在后续目标范围，不因延后删除验收。Web 原列第 2 批，现从近期基础 Insights 范围中移出，详细设计及排期另定；Android/iOS 保留第 3 阶段并分别验证，不把三端 L1 当基础 Insights 的条件。供应商容量/成熟度不能替代 TAP 实测。测试应用/账号/数据、私网、iOS 签名/设备仍由开发、QA、运维确认。
