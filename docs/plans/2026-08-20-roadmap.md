...
> **2026-09-15 应用边界**：Tapper、Knowledge Graph、AI 资产和 AI Test Design 的当前实现迁入独立 TAP AI 前后端；Web LCA 与测试分析原型保留 TAP 非 AI 应用入口。此目录迁移不改变 V0–VG 门禁状态，也不把本机 Demo 升级为生产部署。见 [ADR-027](../decisions/2026-09-15-adr-027-tap-ai-product-app-boundary.md)。
>
> **2026-09-18 AI 编排目标**：[ADR-029](../decisions/2026-09-18-adr-029-langgraph-ai-interaction-task-orchestrator.md) 已接受 TAP AI Chat 与 AI Task 的统一 LangGraph 入口，以及 Fast Chat、Durable Workflow、Bounded Agentic Task 三种执行剖面；Milvus 承载文档检索投影，MySQL 承载可选的有界 active Graph Snapshot 扩展和 Task/GraphRun/checkpoint，TAP Insights API/ClickHouse 承载历史指标。RFC-006/ADR-018 的 legacy loopback Codex 回答组合不在 Project API 作用域内。该目标不改写 V0–V3 已有门禁事实，也不在没有实施计划和质量出口时把 LangGraph 标记为已交付。
...
仓库已从最初的本地知识切片和纯前端原型推进到 V2/V3 主体实现，当前门禁重新打开：

- V0、V1 Gate Review 已通过；V2/V3 已有 Knowledge Graph、Citation、Test Plan 和 Web/API 主体实现，但原 Gate 结论已被[更正评审](../reviews/2026-09-14-v2-v3-gate-correction.md)撤销。
- V2 多 Revision Graph 一致性、V3 真实模型候选的人审绑定和完整 Web 评审闭环待补；V4 暂不放行。
- 产品壳仍保留尚未实施模块的 fixture 和模拟 Run；这些原型状态不能作为 V4/V5 或正式 Execution Evidence。

当前没有产品登录/RBAC、多 Project 产品化、OCR、正式 Automation/Test IR、Recorder、Playwright Bundle、Jenkins Provider 或生产加固；固定 Validation 身份和 loopback 运行不能直接用于公司共享生产环境。旧原型中的 Mobile 和 Azure DevOps 只保留为历史交互探索。
...
V0 自 2026-09-05 启动，已完成 authoritative metadata、[固定身份](../reviews/2026-09-05-tapper-v0-identity-review.md)、[`0007` Project 数据隔离](../reviews/2026-09-05-tapper-v0-project-scope-review.md)及[统一事件/错误契约](../reviews/2026-09-06-tapper-v0-contracts-review.md)的实施与定向验收。各 Review 保留原始失败及修正后的覆盖证据。[Task 3](../reviews/2026-09-06-tapper-v0-http-review.md) 已接入 Project API、Origin 和 Validation Mode 提示；[Task 3A Audit](../reviews/2026-09-06-tapper-v0-audit-review.md) 基础与完整隔离回归通过；[Task 4 有界恢复](../reviews/2026-09-06-tapper-v0-recovery-review.md)已完成并记录原始完整回归失败及测试修正证据，[Task 5 对象存储](../reviews/2026-09-06-tapper-v0-object-storage-review.md)与当前原型真实上传完成定向验收，[Task 5A 隔离 Parser](../reviews/2026-09-06-tapper-v0-parser-isolation-review.md)已完成定向验收，[Task 5B 完整门禁](../reviews/2026-09-06-v0-validation-scope-reliability-gate.md)已通过。
...
[Task 6–9](2026-09-04-tapper-knowledge-web-automation-platform.md#v1--trusted-knowledge-and-durable-conversation) 的 Source、ModelGateway、AI Agent/Skill、Conversation 与真实 Web 接线均已完成；[Task 10 QUALITY-KB-01](../reviews/2026-09-09-v1-trusted-knowledge-gate.md) 于 2026-09-13 通过全部硬阈值、真实 Milvus、隔离 E2E 与完整回归，V1 出口完成。
...
[Task 11–14](2026-09-04-tapper-knowledge-web-automation-platform.md#v2--grounded-knowledge-graph) 的主体实现已存在；后续审查发现多 Document Revision 的 Graph Snapshot 契约和 E2E 证据不足，V2 当前为 `gate-reopened`，关闭条件见[更正评审](../reviews/2026-09-14-v2-v3-gate-correction.md)。
...
[Task 15–17](2026-09-04-tapper-knowledge-web-automation-platform.md#v3--ai-test-design) 的主体实现已存在；后续审查发现质量 profile、人审 digest 绑定和 Web 旅程证据不足，V3 当前为 `gate-reopened`，V4 暂不放行。关闭条件见[更正评审](../reviews/2026-09-14-v2-v3-gate-correction.md)。
...
- `QUALITY-TEST-01`：至少 50 个真实业务意图、一名具名 Reviewer、Schema/BDD 通过率 100%、无来源事实 0、关键需求覆盖 ≥90%、无 Critical Correction Draft ≥80%。
...
Mobile/Appium、Azure DevOps、BrowserStack、Git Sync、企业 SSO、专用 Graph DB、Kubernetes HA 与多企业 SaaS 不在当前路线。统一 AI 交互与任务的 LangGraph 目标已由 ADR-029 决策，但 Fast Chat、Durable Workflow、Bounded Agentic Task 的实施排期、迁移顺序和质量门禁仍需在 RFC-011 及后续实施计划中闭合；在此之前不计入已完成里程碑。其他新能力仍需独立 RFC/ADR，且不得弱化 Project Policy、人工发布、严格 1:1、Test IR 或统一 Evidence。

2026-09-22 范围说明：[RFC-011](../proposals/2026-09-17-rfc-011-rag-test-design-cross-platform-automation.md#迁移或发布方式) 近期只聚焦知识问答与基础 Test Insights，对应其原第 1、2 阶段的部分能力；先用已有 CI 或外部真实报告验证指标链，不以自建 Web/App 自动化完成为前提。其测试管理扩展、执行、完整质量分析/协作、负载与完整主动 Agent 闭环保留后续目标，详细设计和实施计划待补。该提案优先级调整不变更本路线 V0–VG/P0–P1 的门禁或已记录完成状态。
