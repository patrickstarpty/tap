...
>
> **2026-09-18 Chat 编排目标**：[ADR-028](../decisions/2026-09-18-adr-028-langgraph-unified-chat-orchestrator.md) 已接受所有 Tap AI Project Chat 的统一 LangGraph 入口、图内 fast path 和受预算限制的 agentic loop，以 Milvus 承载文档检索投影、MySQL 承载可选的有界 active Graph Snapshot 扩展、TAP Insights API/ClickHouse 承载历史指标。RFC-006/ADR-018 的 legacy loopback Codex 回答组合不在 Project API 作用域内。该目标不改写 V0–V3 已有门禁事实，也不在没有实施计划和质量出口时把 LangGraph 标记为已交付。
...
Mobile/Appium、Azure DevOps、BrowserStack、Git Sync、企业 SSO、专用 Graph DB、Kubernetes HA 与多企业 SaaS 不在当前路线。统一 Chat 的 LangGraph 目标已由 ADR-028 决策，但实施排期、迁移顺序和质量门禁仍需在 RFC-011 及后续实施计划中闭合；在此之前不计入已完成里程碑。其他新能力仍需独立 RFC/ADR，且不得弱化 Project Policy、人工发布、严格 1:1、Test IR 或统一 Evidence。
