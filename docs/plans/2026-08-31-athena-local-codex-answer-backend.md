...
**Spec:** `docs/proposals/2026-08-31-rfc-006-athena-local-codex-answer-backend.md`
...
- ingestion 失败日志只在 durable fail/retry 持久化成功后发出一次 `athena.ingestion.stage_failed`，只含 ID、attempt、stage、安全错误码、闭合诊断码、闭合异常类别和 monotonic duration；不得记录异常正文、输入、文件名、endpoint、response、key、向量、query、Evidence、Codex JSONL 或回答。
- 所有路径保持 UTF-8。自动测试必须覆盖中文问题检索英文来源、英文问题检索中文来源和中英文混合术语；回答跟随问题主要语言，保留必要原文术语，并保持每个实质 claim 至少一个可解析 citation。
- Codex CLI 是本地调用入口而非本地推理；query 与所选 Evidence 会发往 OpenAI，Embedding 内容会发往百炼。该边界只获准用于 loopback、无认证、单操作者 Athena Lab，不能形成 LAN、共享或生产声明。
...
- `apps/backend/src/tap/entrypoints/athena_runtime.py`：`.env` 解析、Embedding/回答 factory、资源 ownership、backend-aware readiness 与 API/worker 分离。
...
- `scripts/check-athena-demo.py`：始终验证 embedding alias，并按 answer backend 验证 LiteLLM chat alias 或原生 Codex/登录/能力。
- `scripts/run-athena-dev.sh` 与 `scripts/run-athena-e2e.sh`：真实 `.env` 透传、secret 收敛和 fake/Codex 互斥；不向 Codex 子进程注入 provider secret。
...
- `apps/backend/tests/smoke/test_athena_codex_smoke.py`：唯一显式 opt-in 的 `0.149.0 + gpt-5.6-sol + ultra` capability/grounded answer gate。
...
- Produces: `_SafeStageError.bind(work, diagnostic_code, exception_type)`, one allowlisted `athena.ingestion.stage_failed` log after durable persistence, and no event when persistence loses the lease.
...
async def test_stage_failure_logs_one_redacted_event_after_durable_failure(caplog) -> None:
    ...
    records = [r for r in caplog.records if r.msg == "athena.ingestion.stage_failed"]
...
async def test_stage_failure_does_not_log_when_failure_persistence_loses_lease(caplog) -> None:
    ...
    assert not [r for r in caplog.records if r.msg == "athena.ingestion.stage_failed"]
```
...
- Modify: `apps/backend/src/tap/entrypoints/athena_runtime.py:300-320,510-590,1035-1080`
...
uv run --project apps/backend pytest \
  ...
  apps/backend/tests/unit/entrypoints/test_athena_runtime.py -v
```
...
git add apps/backend/src/tap/modules/knowledge/ports/search.py \
  ...
  apps/backend/src/tap/entrypoints/athena_runtime.py \
  apps/backend/tests/contract/test_knowledge_api.py \
  apps/backend/tests/contract/test_authorized_execution.py \
  apps/backend/tests/contract/test_resource_modes.py \
  apps/backend/tests/integration/milvus_runtime.py \
...
corepack pnpm --filter @tap/web test -- AthenaWorkspace.test.tsx --run
```
...
git add apps/backend/src/tap/modules/knowledge/adapters/grounded_output.py \
  ...
  apps/web/src/widgets/athena/AthenaWorkspace.test.tsx
...
- Modify: `apps/backend/src/tap/entrypoints/athena_runtime.py:55-320`
- Modify: `apps/backend/tests/unit/entrypoints/test_athena_runtime.py`

**Interfaces:**
- Consumes: `AthenaSettings.from_mapping`, `shutil.which("codex")` for discovery only, current platform `system/machine`, uid and an expected exact version.
...
def test_answer_backend_defaults_to_litellm_without_codex_discovery(monkeypatch) -> None:
    ...
    settings = AthenaSettings.from_mapping(valid_environment())
...
**Files:**
- Modify: `apps/backend/src/tap/entrypoints/athena_runtime.py:300-320,510-590,688-730,863-1035,1035-1115`
- Modify: `apps/backend/tests/unit/entrypoints/test_athena_runtime.py`
- Modify: `apps/backend/tests/integration/test_ingestion_entrypoint.py`
...
- Produces: `AthenaEmbeddingPort`, `AthenaAnswerBackend`, `_create_embeddings`, `_create_answer_backend`, API-only Codex ownership/readiness, worker-only LiteLLM Embedding, and backend-aware `demo-check`.
...
async def test_codex_api_uses_litellm_embeddings_and_codex_answers(monkeypatch) -> None:
    embeddings = RecordingAthenaEmbeddings()
...
Add readiness cases: LiteLLM requires `{athena-embedding, athena-chat}`; Codex requires only `athena-embedding` plus successful `codex.check_ready()`; failed Codex login/version/feature makes `models` failed; liveness stays `ok`; relay and collection commands parse Codex settings without discovery; E2E rejects Codex and never invokes a real provider.
...
Use one `_create_litellm_adapter(settings) -> LiteLLMAdapter` helper. `_create_embeddings(settings)` returns `DeterministicAthenaModel` only for exact E2E, otherwise that LiteLLM adapter. `_create_answer_backend(settings, *, embeddings)` returns the same object with no second owner for fake/LiteLLM, or resolves `shutil.which("codex")`, the native target and the real `CODEX_HOME`, then returns a new `CodexExecAnswerAdapter` for Codex.
...
def _create_answer_backend(
    ...
    *,
    ...
    return AthenaAnswerBackend(adapter, adapter.check_ready, adapter)
```
...
`scripts/check-athena-demo.py::_check_models` must use the same label set and native Codex non-generating readiness helper, close every constructed owner, and still print exactly `models ok` or `models failed configure-models`. `_PROVIDER_SETTINGS` remains `("DASHSCOPE_API_KEY",)` for both real modes.
...
git add .env.example Makefile compose.yaml deploy/local/litellm/config.yaml \
  ...
  apps/backend/tests/integration/test_relay_entrypoint.py \
  apps/backend/tests/contract/test_demo_commands.py \
  scripts/check-athena-demo.py scripts/run-athena-dev.sh scripts/run-athena-e2e.sh
git commit -m "feat: select athena answer backend from env"
```
...
- Create: `apps/backend/tests/smoke/test_athena_codex_smoke.py`
- Modify: `apps/backend/tests/smoke/test_athena_real_model.py`
...
Extend the existing `TAP_RUN_ATHENA_REAL_MODEL_SMOKE=1` test with only fictional strings:
...
Require model alias `athena-embedding`, dimension `1536`, finite values and valid usage for all six calls. Emit only alias, direction pass/fail and monotonic milliseconds; never emit the strings or vectors.
...
async def test_local_codex_ultra_is_single_agent_tool_free_and_grounded(
    ...
    monkeypatch.setenv("ATHENA_CODEX_SENTINEL_PATH", str(sentinel_file))
    monkeypatch.setattr(
        codex_target,
...
```sh
env -u TAP_RUN_ATHENA_REAL_MODEL_SMOKE -u TAP_RUN_ATHENA_CODEX_CONFORMANCE \
  uv run --project apps/backend pytest \
  apps/backend/tests/smoke/test_athena_real_model.py \
...
```sh
TAP_RUN_ATHENA_CODEX_CONFORMANCE=1 \
  uv run --project apps/backend pytest \
  apps/backend/tests/smoke/test_athena_codex_smoke.py -v -rs
```
...
git add apps/backend/src/tap/modules/knowledge/adapters/codex_target.py \
  ...
  apps/backend/tests/contract/test_cross_language_retrieval.py \
  apps/backend/tests/smoke/test_athena_real_model.py \
  apps/backend/tests/smoke/test_athena_codex_smoke.py
...
```sh
TAP_RUN_ATHENA_REAL_MODEL_SMOKE=1 \
  uv run --project apps/backend pytest \
...
paths = [
    ...
    Path("docs/proposals/2026-08-31-rfc-006-athena-local-codex-answer-backend.md"),
    Path("docs/plans/2026-08-31-athena-local-codex-answer-backend.md"),
]
missing: list[str] = []
for source in paths:
