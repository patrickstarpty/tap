...
> **当前聚焦（2026-09-22）：知识问答与基础 Test Insights。** 分别对应原第 1、2 阶段中的部分能力，不表示这两个阶段的全部功能都纳入近期交付。本文及配套专题已有总体流程、技术选择和部分契约约束，尚不是五个阶段均可直接实施的完整详细设计；具体范围与待补设计见“迁移或发布方式”。后文完整业务场景、主动 Agent 和架构图仍展示平台目标全貌。
...
近期验证两条路径：一是上传、核对和发布资料后，获得有出处的知识回答；二是接入已有 CI 或外部真实测试报告，通过基础 Test Insights 查看指标、定位失败，再由 TAP AI 结合知识解释原因。基础 Insights 不以平台自建 Web / App 自动化完成为前提。
...
主动 Agent 保留为平台目标亮点，当前先聚焦知识问答与基础 Insights，不将完整订阅、建议收件箱和自动行动闭环列为这两条路径的交付前置条件。后续先做知识变更影响建议，再接真实 Run 和 Insights 扩展回归与失败跟进；订阅可暂停，建议可忽略或延后，噪声和实际收益纳入试点评估。
...
主文说明产品与架构决策，以下专题保留已有技术设计与验收约束，状态与本 RFC 一致。专题有内容不等于详细设计已闭合：近期细化知识问答和基础 Insights，其余能力保留目标设计并标注待补内容。
...
**当前只推进知识问答与基础 Test Insights 的设计和验证。** 保留原阶段编号便于追溯，但收窄第 1、2 阶段的近期范围。以下标的是设计深度与优先级，不是完成清单或工期承诺。

| 原路线定位 | 当前安排 | 已有设计与待补内容 |
| --- | --- | --- |
| 1. 可信知识与测试设计 | **近期：知识问答。** 资料上传/解析、核对发布、检索与引用、多轮问答、必要的长任务恢复 | 已有解析、检索、审核、编排与质量约束；待按试点资料细化首批文件类型、接口/状态迁移、交互异常及实施任务。独立用例、测试生成扩展、手工执行与 Jira 写入留待后续 |
| 2. Web 与基础 Insights | **近期：基础 Test Insights。** 真实报告接入、去重、ClickHouse、Insights API、运行/失败/趋势视图及 AI 解读 | 已有数据链、指标口径、身份去重和跨产品边界；待细化首个报告来源与样本映射、表结构/API、页面交互和实施任务。平台自建 Web 调试、脚本生成和执行闭环留待后续 |
| 3. Android / iOS | **后续阶段，详细设计待补** | 已有平台选型、调试流程和真实证据约束；设备接入、会话协议、平台动作、签名/安装、调度恢复与分平台实施计划尚需细化 |
| 4. 完整 Insights 与协作 | **后续阶段，详细设计待补** | 已有五类质量域、规则和协作目标；逐连接器映射、完整质量模型、告警/发布检查及 Forge Jira 内嵌交互尚需细化；不与近期基础 Insights 混同 |
| 5. Load Testing | **后续阶段，详细设计待补** | 已有 k6 方向、负载模型、统计和停止约束；执行池、发压调度、分布式汇总、私网部署及容量验收实施计划尚需细化 |

近期两条路径的验证边界：

- **知识问答：** 真实资料可核对、版本可追溯；回答引用可回到原文；无依据、解析缺失与权限不足明确显示；必要长任务可安全恢复。测试用例生成与自动化执行不作为本轮完成条件。
- **基础 Test Insights：** 先用已有 CI 或外部真实报告打通“原件/接收账本 → 校验映射去重 → ClickHouse → TAP Insights API → 看板/AI 解读”；验证首次/最终通过率、失败分布、耗时与趋势，能够钻取原始记录，重复报告不重复计数、缺数据不伪装正常。首个来源、指标集合与查询性能目标在详细设计中固定。

完整 Skills/Agents 资源包、主动订阅与建议收件箱、正式回归调度、完整 RCA 自动跟进均为后续扩展；近期只建设知识和指标查询所需的受控 AI 能力。测试管理、Web 自动化及第 3–5 阶段启动前，须补齐对应详细设计与实施计划，不能仅凭本路线表开工或宣称具备全部能力。
